...
void TestPStrip(Host *host){
    ...
    if(dataLinkLayerReceive(host)){         // Check and accept packets addressed to self_mac
...
void physicalLayerTransfer(Host host[],int num_hosts)
{
    for(int i = 0; i < num_hosts;++i)
    {
        ...
        if(pkt_len < 6) continue; // no valid frame

        char dest_mac = host[i].buf[0][1];
        int dest_index = dest_mac - 'A';
        if(dest_index < 0 || dest_index >= num_hosts) continue;     // invalid dest
